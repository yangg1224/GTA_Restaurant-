...
```{r restaurant_type, fig.cap = "Restaurant types", echo=FALSE,fig.width=12, fig.height=4}
...
```
Two merged histograms charts were used to illustrate the restaurant's revenue distribution. 

(Figure \@ref(fig:revenue_distribution)) 
...
### Correlation matrix
```{r correlation, fig.cap = "Correlation matrix",echo=FALSE, fig.width=20, fig.height=10}
#install.packages("fastDummies")
library(fastDummies)
Dummy_data<-treated_data[-c(1,2)] #Remove Type and postcode columns
...
# correlation matrix for restaurants in control group
Dummy_data<-control_data[-c(1,2)] #Remove Type and postcode columns

Dummy_data<- fastDummies::dummy_cols(Dummy_data) #  change categorical var to factors 


Dummy_data<-Dummy_data[-c(1:3,5:7,9:11,13,44)]
...
ggarrange(
  ...
  labels = c('Treated group', 'Control group'),
  ncol = 2, nrow = 1,
  align = "hv"
  )
